...
$q1=  dbConnect()->prepare("SELECT * FROM payment WHERE custID='$ord'");
$q1->execute();
$ro=$q1->fetch();

$tot_paid =  dbConnect()->prepare("SELECT sum(amount) AS amount FROM payment WHERE custID='$ord'");
$tot_paid->execute();
$ry=$tot_paid->fetch();
$tot = $ry['amount'];

$q= dbConnect()->prepare("SELECT * FROM subscribers WHERE subscriberId='$ord'");
$q->execute();
$r=$q->fetch();
        ...
        <div class="hk-pg-wrapper">
            ...
            <div class="container">
                ...
                <div class="hk-pg-header mb-10">
                    <div>
                        <a href="#saleInv?order=<?php echo $ord; ?>" title="View Contact of Sale"> <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="book"></i></span></span><?php echo $r['firstname']." ".$r['lastname']; ?></h4></a>
                    </div>
		   <div class="d-flex">
                     <?php if($bln > 0) {echo "  <a href='obalance?order=$ord' class='btn btn-info text-secondary mr-15'><span style='color: #fff;'>Add Balance</span></a>" ;}?>
                       <a  onclick="printContent('print')" class="text-secondary mr-15"><span class="feather-icon"><i data-feather="printer"></i></span></a>
                    </div>
                </div>
                <!-- /Title -->
                            ...
                            <div class="invoice-from-wrap">
                                <div class="row">
                                    <div class="col-md-7 mb-20">
                                        <img class="img-fluid invoice-brand-img d-block mb-20" src="<?php echo $lgg; ?>" alt="brand" />
...
<!--                                        <h4 class="mb-35 font-weight-600">Invoice / Receipt</h4>-->
                                        <span class="d-block">Transaction Date:<span class="pl-10 text-dark"><?php echo $ro['datepaid']; ?></span></span>
                                        <span class="d-block">Invoice:<span class="pl-10 text-dark"><?php echo $ord; ?></span></span>
                                        <span class="d-block">Customer:<span class="pl-10 text-dark"><?php echo $r['firstname']." ".$r['lastname']; ?></span></span>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-border mb-0">
                                            ...
                                            <tbody>
                                                <?php 
                                                   $py = 0;
                                                   $qx=  dbConnect()->prepare("SELECT * FROM payment WHERE custID='$ord'"); 
                                                   $qx->execute();
                                                   while($rx=$qx->fetch()){
                                                   $py += $rx['amount'];   
                                                   //$bal = $amt - $py;
                                            ...
                                            <tfoot class="border-bottom border-1">
                                                <tr>
                                                    ...
                                                    <th class="text-right font-weight-600"> <?php echo number_format($tot); ?></th>
                                                </tr>
                                                <tr>
                                                    <?php 
                                                    $bal = ($amt - $tot);  

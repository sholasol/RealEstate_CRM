...
if(isset($_POST['save'])){
    ...
    elseif(empty($_POST['rate'])){
        ...
        $q1=  dbConnect()->prepare("SELECT count(id) AS no FROM stax WHERE tax='$tax'");
        $q1->execute();
        $rr=$q1->fetch();
        $no=$rr['no'];
        ...
        if($no < 1){
            ...
            $q=  dbConnect()->prepare("INSERT INTO stax SET tax=:tax, rate=:rate, created=:dt, createdby=:by ");
        $q->bindParam(':tax', $tax);
        $q->bindParam(':rate', $rate);
        $q->bindParam(':dt', $dt);
        $q->bindParam(':by', $user);
        if($q->execute()){
            $in= dbConnect()->prepare("INSERT INTO activity SET userID='$uid', activity='Creation of sales tax by userID $uid', created=now()");
            $in->execute();
            ...
            echo"
                    ...
                    <div style='width:100%;text-align:center;vertical-align:bottom'>
                            ...
                            <br>
                            <meta http-equiv='refresh' content='1;url=stax'>
        ...
        <div class="hk-pg-wrapper">
            ...
            <div class="container mt-xl-50 mt-sm-30 mt-15">
                ...
                <div class="hk-pg-header align-items-top">
                    <div>
                        <h4 class="hk-pg-title font-weight-600 mb-10">Sales Taxes</h4>
                    </div>
                </div>
                <!-- /Title -->
                                 ...
                                 <div class="table-wrap">
                                            <div class="table-responsive">
                                                    <table class="table table-sm table-hover mb-0">
                                                            ...
                                                            <tbody>
                                                                <?php 
                                                                $q=  dbConnect()->prepare("SELECT * FROM stax");
                                                                $q->execute();
                                                                $counter =0;
                                                                while($ro=$q->fetch()){
                                                                ?>

...
$chk =dbConnect()->prepare("SELECT count(order_no) AS order_no FROM sinvoice WHERE order_no='$order'");
$chk->execute();
$rr=$chk->fetch();
$orderNo =$rr['order_no'];

if($orderNo < 1){
     $e="Please create invoice for this order before processing payment"; 
     echo  " <script>alert('$e'); window.location='saleInv?order=$order'</script>"; 
}

        ...
        <div class="hk-pg-wrapper">
            ...
            <div class="container mt-xl-50 mt-sm-30 mt-15">
                ...
                <div class="hk-pg-header align-items-top">
                    <div>
                        <h4 class="hk-pg-title font-weight-600 mb-10">New Deposit</h4>
                    </div>
                </div>
                <!-- /Title -->
                       ...
                       <form class="needs-validation" method="post" novalidate>
                            <div class="form-row">
                                <div class="col-md-12 mb-20">
                                    <select name="dep" class="form-control custom-select d-block w-100"  required>
                                        <option value="">Choose Name</option>
                                        <?php
                                        $b=  dbConnect()->prepare("SELECT * FROM customer");
                                        $b->execute();
                                        while($r=$b->fetch()){
                                        ?>
                                        <option value="<?php echo $r['custID']; ?>"><?php echo $r['fname']." ".$r['lname']; ?></option>
                                        <?php } ?>
                                    </select>
                                    <div class="valid-feedback"> Looks good! </div> <div class="invalid-feedback"> Error </div>
                                </div>
                                <div class="col-md-12 mb-20">
                                    <select name="bank" class="form-control custom-select d-block w-100" id="bank" required>
                                        <option value="">Choose Bank</option>
                                        <?php
                                        $b=  dbConnect()->prepare("SELECT * FROM bank");
                                        $b->execute();
                                        while($r=$b->fetch()){
                                        ?>
                                ...
                                <div class="col-md-12 mb-20">
                                    <input type="text" class="form-control" id="validationCustom01" name="acct" id="acct" placeholder="Account Number"  required>
                                    <div class="valid-feedback"> Looks good! </div> <div class="invalid-feedback"> Error </div>
                                </div>
                                <div class="col-md-12 mb-20">
                                 ...
                                 <div class="table-wrap">
                                            <div class="table-responsive">
                                                    <table id="datable_1" class="table table-hover w-100 display pb-30">
                                                            ...
                                                            <tbody>
                                                                <?php 
                                                                $q=  dbConnect()->prepare("SELECT * FROM deposit WHERE received='$uid' AND created=CURDATE()");
                                                                $q->execute();
                                                                $counter =0;
                                                                while($ro=$q->fetch()){
                                                                    $dp=$ro['depositor'];
                                                                    ...
                                                                    $empN = dbConnect()->prepare("SELECT * FROM customer WHERE custID='$dp'");
                                                                    $empN->execute();
                                                                    $x= $empN->fetch();
                                                                    $custName=$x['fname']." ".$x['lname'];

                                                                    $bn= dbConnect()->prepare("SELECT * FROM bank WHERE id='$bnk'");
                                                                    $bn->execute();
                                                                    $rw=$bn->fetch();
                                                                    $bName =$rw['bank'];
                                                                ?>
                                                                    <tr>
                                                                            ...
                                                                            <td><?php echo number_format($ro['amount'], $decimal=2); ?></td>
                                                                    </tr>
                                                                <?php } ?>
                                                            </tbody>
                                                    </table>

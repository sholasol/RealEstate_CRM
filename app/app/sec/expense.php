...
//Sales Period for chart
            ...
            //expense
            $ss = dbConnect()->prepare("Select sum(total) as amount, sum(qty) AS qty From expense Where branch='$branch' AND YEAR(created)= Year(CURRENT_DATE())");
            $exp=$ss;
            $exp->execute();
            $ra=$exp->fetch();
            $exep = $ra['amount'];
                            ...
                            <div class="card card-sm">
                                    <div class="card-body">
                                            ...
                                            <div class="d-flex align-items-end justify-content-between">
                                                    <div>
                                                            <span class="d-block">
                                                                    <span class="display-5 font-weight-400 text-dark"><?php echo number_format($qty) ;?></span>
                                                            </span>
                                                    </div>
                                            </div>
                                    </div>
                               ...
                               <table id="datable_1" class="table table-hover w-100 display pb-30">
                                ...
                                <tbody>
                                    <?php 
                                        ...
                                        $q=  dbConnect()->prepare("SELECT * FROM expense WHERE branch='$branch' AND createdby='$uid'");
                                        $q->execute();
                                        $counter =0;
                                        while($ro=$q->fetch()){
                                            $u=$ro['createdby'];

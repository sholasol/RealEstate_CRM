...
if(isset($_POST['save'])){
    ...
    if(empty($_POST['exp'])){
        ...
        $q1=  dbConnect()->prepare("SELECT count(id) AS no FROM leave_type WHERE type='$exp'");
        $q1->execute();
        $rr=$q1->fetch();
        $no=$rr['no'];
        ...
        $q=  dbConnect()->prepare("INSERT INTO leave_type SET type=:exp ");
        $q->bindParam(':exp', $exp);
        if($q->execute()){
            $in= dbConnect()->prepare("INSERT INTO activity SET userID='$uid', activity='Creation of leave type ($exp) by userID $uid', created=now()");
            $in->execute();
                                                            ...
                                                            <tbody>
                                                                ...
                                                                ?>
                                                                    <tr>
                                                                            ...
                                                                            <td>
                                                                                <a href="EleaveType?id=<?php echo $lid; ?>" class="btn btn-info" style="color: white;" title="Edit"><i class="fa fa-edit"></i></a>
                                                                                <?php // echo $ro['rate']; ?>
                                                                            </td>
                                                                    </tr>
                                                                <?php } ?>

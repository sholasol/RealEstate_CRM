...
$qq=  dbConnect()->prepare("SELECT sum(amount) AS amount, sum(qty) AS plots, sum(paid) AS paid, sum(balance) AS bal FROM sorder  ");
$qq->execute();
$ro=$qq->fetch();
                               ...
                               <table id="datable_1" class="table table-hover w-100 display pb-30">
                                ...
                                <tbody>
                                    <?php 
                                        ...
                                        $q=  dbConnect()->prepare("SELECT  amount, balance, custID  AS cust, item, qty, paid, balance, created, code, order_no, kfn, kln, type FROM sorder WHERE branch='$branch' GROUP BY code ORDER BY id DESC");
                                        $q->execute();
                                        $counter =0;
                                        while($ro=$q->fetch()){
                                            $u=$ro['cust'];
                                        ...
                                        <tr>
                                                ...
                                                <td>
                                                    <?php echo $ro['item']; ?><br><small> <?php echo $ro['type']; ?></small>
                                                </td>
                                                <td><span class="badge badge-primary"><?php echo number_format($ro['qty'])." Unit(s)"; ?></span></td>

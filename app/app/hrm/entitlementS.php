...
if(isset($_POST['save'])){
    ...
    elseif(empty($_POST['grade'])){
        ...
        $q1=  dbConnect()->prepare("SELECT count(id) AS no FROM emp_tax WHERE tax='$tax' AND grade='$grade'");
        $q1->execute();
        $rr=$q1->fetch();
        $no=$rr['no'];
        ...
        $q=  dbConnect()->prepare("INSERT INTO emp_tax SET tax=:tax, grade=:grade, rate=:rate, created=:dt, createdby=:by ");
        $q->bindParam(':tax', $tax);

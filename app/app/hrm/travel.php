        ...
        //All request
        $db = dbConnect()->prepare("SELECT count(id) AS num, sum(fund) AS fund FROM `travel` ");
        $db->execute();
        $ro=$db->fetch();
        ...
        //Granted
        $db1 = dbConnect()->prepare("SELECT  sum(fund) AS fund FROM `travel` WHERE status='Approved' ");
        $db1->execute();
        $rw=$db1->fetch();
        $granted = $rw['fund'];
        
        //Pending
        $db2 = dbConnect()->prepare("SELECT  sum(fund) AS fund FROM `travel` WHERE status='Pending' ");
        $db2->execute();
        $rr=$db2->fetch();
        $pending = $rr['fund'];
        ?>
                            ...
                            <div class="card-body pa-0">
                                <div class="table-wrap"><br>
                                        <table id="datable_1" class="table table-hover w-100 display pb-30">
                                            ...
                                            <tbody>
                                                <?php
                                                $tr=  dbConnect()->prepare("SELECT * FROM travel WHERE branch='$branch'");
                                                $counter=0;
                                                $tr->execute();
                                                while($r=$tr->fetch())
                                                {
                                                            ...
                                                            <div class="btn-group">
                                                                    <div class="dropdown">
                                                                        ...
                                                                        <div role="menu" class="dropdown-menu">
                                                                            <a class="dropdown-item" href="travelP?id=<?php echo $id; ?>">Process Request</a>
                                                                            <!--

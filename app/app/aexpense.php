...
if(isset($_POST['save'])){
    ...
    elseif(empty($_POST['amt'])){
     ...
     elseif(empty($_POST['type'])){
        ...
        $q=  dbConnect()->prepare("INSERT INTO expense SET item=:item, qty=:qty, amount=:amt, type=:type, total=:total, note=:note, created=:dt, createdby=:by, branch=:brn, code=:code ");
                                 ...
                                 <div class="table-wrap">
                                            <div class="table-responsive">
                                                    <table class="table table-sm table-hover mb-0">
                                                            ...
                                                            <tbody>
                                                                <?php 
                                                                $q=  dbConnect()->prepare("SELECT * FROM expense WHERE createdby='$uid' AND created=CURDATE()");
                                                                $q->execute();
                                                                $counter =0;
                                                                while($ro=$q->fetch()){

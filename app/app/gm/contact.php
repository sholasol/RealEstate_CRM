<?php 
        ...
        $total_order=dbConnect()->prepare("SELECT sum(qty) AS no FROM sorder WHERE YEAR(created)= YEAR(CURRENT_DATE()) ");
        $total_order->execute();
        $rr=$total_order->fetch();
        $num = $rr['no'];
        
        $rv = dbConnect()->prepare("SELECT sum(total) AS tot FROM sorder WHERE YEAR(created)= YEAR(CURRENT_DATE()) ");
        $rv->execute();
        $rw=$rv->fetch();
        $tot = $rw['tot'];
        
        $rv2 = dbConnect()->prepare("SELECT sum(balance) AS tot FROM sorder  WHERE YEAR(created)= YEAR(CURRENT_DATE())");
        $rv2->execute();
        $rx=$rv2->fetch();
        $tot2 = $rx['tot'];
        ?>
...
<!--                        <div class="btn-group btn-group-sm btn-group-rounded mb-15 mr-15" role="group">
                        ...
                        <a href="addContact" class="btn btn-sm btn-outline-secondary btn-rounded btn-wth-icon icon-wthot-bg mb-15"><span class="icon-label"><span class="feather-icon"><i data-feather="user"></i></span> </span><span class="btn-text">New Customer</span></a>
                    </div>
                </div>
                <!-- /Title -->
                    ...
                    <div class="col-xl-12">
                        <div class="hk-row">
                            ...
                            <div class="col-lg-3 col-sm-6">
                                    <div class="card card-sm">
                                            <div class="card-body">
                                                    <span class="d-block font-11 font-weight-500 text-dark text-uppercase mb-10">Plots Sold</span>
                                                    <div class="d-flex align-items-center justify-content-between position-relative">
                                                            <div>
                                                                    <span class="d-block">
                                                                ...
                                                                <div class="btn-group">
                                                                    <div class="dropdown">
                                                                        ...
                                                                        <div role="menu" class="dropdown-menu">
                                                                            ...
                                                                            <?php
                                                                            if($role=='Admin'){ echo"
                                                                            <a class='dropdown-item' href='cust_edit?id=$id'>Edit Customer</a>

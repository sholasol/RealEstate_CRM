                               ...
                               <table id="datable_1" class="table table-hover w-100 display pb-30">
                                ...
                                <tbody>
                                    <?php 
                                        ...
                                        $q=  dbConnect()->prepare("SELECT  * FROM subscribers WHERE amount_agreed > 0");
                                        $q->execute();
                                        $counter =0;
                                        $py = 0;
                                        while($ro=$q->fetch()){
                                            ...
                                            $empN = dbConnect()->prepare("SELECT * FROM payment WHERE custID='$sid'");
                                            $empN->execute();
                                            $x= $empN->fetch();
                                            
                                            $tot_paid =  dbConnect()->prepare("SELECT sum(amount) AS amount FROM payment WHERE custID='$sid'");
                                            $tot_paid->execute();
                                            $ry=$tot_paid->fetch();
                                            $tot = $ry['amount'];
                                            
                                            $paid = $x['amount'];
                                            $bl = ($ro['amount_agreed'] - $tot);
                                            $order = $sid;
                                        ?>
                                        <tr>
                                                <td><?php echo ++$counter; ?></td>
                                                <td><a class="dropdown-item" href="sview?order=<?php // echo $order; ?>"><?php echo $ro['firstname']." ".$ro['lastname']; ?><br><small><?php echo $ro['phone'];?></small></a></td>
                                                <td>
                                                    <span class="badge badge-light"><?php echo $ro['which_estate']; ?></span>
                                                </td>
                                                <td><span class="badge badge-primary"><?php echo number_format($ro['quantity'])." Plot(s)"; ?></span></td>
                                                <td><span class="badge badge-primary"><?php echo number_format($ro['amount_agreed']); ?></span></td>
                                                <td><span class="badge badge-success"><?php echo number_format($tot); ?></span></td>
                                                <td><span class="badge badge-orange"><?php if($bl > 0){echo number_format($bl);}else{echo"<span class='badge badge-success'>Fully Paid</span>";} ?></span></td>
                                                <td>
                                                    <div class="btn-group">
                                                        <div class="dropdown">
                                                            ...
                                                            <div role="menu" class="dropdown-menu">
                                                                <a class="dropdown-item" href="oview?order=<?php  echo $sid; ?>">View Sale Record</a>
                                                                <?php 
                                                                if($bl > 0){echo "<a class='dropdown-item' href='obalance?order=$order'>Pay Balance</a>";}
                                                                echo "<a class='dropdown-item' href='saleInv?order=$sid'>Contract of Sale</a>";
                                                                echo "<a class='dropdown-item' href='allocation?order=$sid'>Letter of Allocation</a>";
                                                                echo "<a class='dropdown-item' href='acknowledge?order=$sid'>Letter of Acknowledgement</a>";
                                                                echo "<a class='dropdown-item' href='deed?order=$sid'>Deed of assignment</a>";
                                                                echo "<a class='dropdown-item' href='payFee?order=$sid'>Pay Statutory fees</a>";
                                                                ?>
